 ...
 *
 * Cinematic F1 broadcast-style hero for the /predict page.
 *
 * Aesthetic direction: Sky Sports F1 opening title card energy —
 * dramatic angular geometry, speed lines, layered glows, data overlays,
 * and a race countdown ticker. Dark, intense, unmistakably F1.
 *
 * Features:
 *   - Animated speed lines streaming across the background
 *   - Layered diagonal red slash accent (classic F1 livery geometry)
 *   - Live countdown to race day
 *   - Staggered entrance animations on load
 *   - Circuit name rendered as a massive ghost watermark behind the title
 *   - Model summary pill with animated pulse dot
 *   - Scan-line texture overlay for that broadcast monitor feel
 */
"use client";
...
// Accept a timestamp (number) instead of a Date object so the dependency
// never changes reference between renders — fixes the infinite re-render loop.
function useCountdown(raceDateMs: number) {
  const [timeLeft, setTimeLeft] = useState({ days: 0, hours: 0, minutes: 0, past: false });
  ...
  useEffect(() => {
    ...
    function calculate() {
      ...
      setTimeLeft({ days, hours, minutes, past: false });
    }

    calculate();
    const id = setInterval(calculate, 60_000);
    return () => clearInterval(id);
  // raceDateMs is a primitive number — stable across renders, no infinite loop
  }, [raceDateMs]);
...
export default function PredictionHero({ prediction }: PredictionHeroProps) {
  ...
  // Pass .getTime() (a stable number) not the Date object itself —
  // Date objects are new references every render which causes infinite loops
  const countdown     = useCountdown(raceDate.getTime());

  // Extract short circuit label for the ghost watermark
  // e.g. "Miami International Autodrome" → "MIAMI"
  ...
  return (
    <section
      style={{
        ...
        background: "#060606",
        // Minimum height so the hero feels substantial
    ...
    >
      {/* ── Layer 1: Grid texture ─────────────────────────────────────────
          Fine dot-grid gives depth without being distracting              */}
      <div
        style={{
          position: "absolute",
          inset: 0,
      ...
      {/* ── Layer 2: Speed lines ──────────────────────────────────────────
          Horizontal streaks that animate rightward, simulating velocity   */}
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none", overflow: "hidden" }}>
        {[
          { top: "18%",  width: "35%", delay: "0s",    opacity: 0.07, left: "-5%" },
          { top: "32%",  width: "55%", delay: "0.4s",  opacity: 0.05, left: "-5%" },
          { top: "48%",  width: "25%", delay: "0.8s",  opacity: 0.09, left: "-5%" },
          { top: "61%",  width: "45%", delay: "0.2s",  opacity: 0.06, left: "-5%" },
          { top: "74%",  width: "30%", delay: "1.1s",  opacity: 0.04, left: "-5%" },
          { top: "85%",  width: "60%", delay: "0.6s",  opacity: 0.05, left: "-5%" },
        ].map((line, i) => (
          <div
            key={i}
            style={{
      ...
      {/* ── Layer 3: Diagonal red slash ───────────────────────────────────
          The bold angular accent — F1 livery geometry                     */}
      <div
        style={{
          position: "absolute",
          top: "-20%",
      ...
      {/* ── Layer 4: Radial glow ──────────────────────────────────────────
          Warm red glow emanating from top-right                           */}
      <div
        style={{
          position: "absolute",
          top: "-30%",
      ...
      {/* ── Layer 5: Ghost circuit watermark ─────────────────────────────
          Enormous faded circuit name sitting behind the content           */}
      <div
        style={{
          position: "absolute",
          right: "-2%",
      ...
      {/* ── Layer 6: Scan-line overlay ────────────────────────────────────
          Fine horizontal lines — broadcast monitor texture                */}
      <div
        style={{
          position: "absolute",
          inset: 0,
      ...
      >
        ...
        <h1
          style={{
            ...
            // Red accent on last word
            maxWidth: "70%",
          }}
        >
          {/* Split race name — colour the last word red */}
          {(() => {
        ...
        {/* Bottom row: model pill + countdown */}
        <div
          style={{
            display: "flex",
            alignItems: "flex-end",
        ...
        >
          ...
          {!countdown.past && (
            ...
            >
              ...
              <div style={{ color: "rgba(255,255,255,0.15)", fontFamily: "'Russo One', sans-serif", fontSize: "1.2rem", paddingBottom: "0.4rem" }}>:</div>
      ...
      <div
        style={{
          ...
          height: "60%",
          background:
            "linear-gradient(180deg, #E10600 0%, transparent 100%)",
          zIndex: 3,
        }}
      />
